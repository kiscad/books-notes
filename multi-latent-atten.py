...
class LinearNoTP(torch.nn.Linear):
    def __init__(
        ...
        **kwargs,
        ...
        #self.weight = torch.nn.Parameter(torch.empty(output_size, input_size, bias=kwargs.get('bias', True), device=torch.cuda.current_device(), dtype=config.params_dtype))
        # self.weight = torch.nn.Parameter(
        #     torch.empty((output_size, input_size), dtype=config.params_dtype)
        ...
        #     )
        # )
        # self.weight = torch.nn.Linear(output_size, input_size, bias=kwargs.get('bias', True)).to(torch.cuda.current_device())
    ...
    def forward(self, input_):
        ...
        #     output = bnb.matmul_4bit(input_, self.weight, self.weight.quant_state, bias=self.bias)
        # else:
